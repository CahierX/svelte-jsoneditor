...
<script lang="ts">
  ...
  function collapseRecursively(path: JSONPath) {
    try {
      ...
      function findFoldableLocations(currentValue: any, currentPath: JSONPath) {
        if (currentValue && typeof currentValue === 'object' && currentValue !== null) {
          ...
          if (Array.isArray(currentValue)) {
            currentValue.forEach((item, index) => {
              findFoldableLocations(item, currentPath.concat(String(index)))
            })
          } else {
            Object.keys(currentValue as Record<string, any>).forEach((key) => {
              findFoldableLocations((currentValue as Record<string, any>)[key], currentPath.concat(key))
            })
          }
        }
      }

      findFoldableLocations(value, path)
